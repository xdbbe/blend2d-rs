...
use crate::{err_to_result, Error, ExtendMode};
...
impl Gradient {
    #[inline]
    pub fn new(type_: ffi::BLGradientType::Type, values: *const ::std::os::raw::c_void, extend_mode: ExtendMode) -> Self {
        let mut gradient = std::mem::MaybeUninit::<ffi::BLGradientCore>::uninit();
        unsafe {
            ffi::bl_gradient_init_as(gradient.as_mut_ptr(), type_, values, extend_mode as i32, null(), 0, null());
            Gradient(gradient.assume_init())
        }
    }
    #[inline]
    pub fn new_linear(values: &LinearGradientValues, extend_mode: ExtendMode) -> Self {
        Self::new(ffi::BLGradientType::BL_GRADIENT_TYPE_LINEAR, &raw const *values as _, extend_mode)
    }
    #[inline]
    pub fn new_radial(values: &LinearGradientValues, extend_mode: ExtendMode) -> Self {
        Self::new(ffi::BLGradientType::BL_GRADIENT_TYPE_RADIAL, &raw const *values as _, extend_mode)
    }
    #[inline]
    pub fn new_conic(values: &LinearGradientValues, extend_mode: ExtendMode) -> Self {
        Self::new(ffi::BLGradientType::BL_GRADIENT_TYPE_CONIC, &raw const *values as _, extend_mode)
    }
    #[inline]
